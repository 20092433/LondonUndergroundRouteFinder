...
//        }
    ...
    public void routeViewCheapest(){
        ...
        //System.out.println("-------------------------------------");
        String thePathh = "";
        GraphNode<Station> previousStat = null;
        ...
        for(GraphNode<?> n : cpa.pathList) {
            ...
            if(previousStat != null)
                drawLine(previousStat.data.getX(), previousStat.data.getY(), ((Station)n.data).getX(), ((Station)n.data).getY());
            previousStat = (GraphNode<Station>) n;
        }
...
public void drawLine(int x1, int y1, int x2, int y2){
        ...
        L.setStrokeWidth(4);
        L.setTranslateX(mapView.getLayoutX());
        L.setTranslateY(mapView.getLayoutY());
    ((AnchorPane )mapView.getParent()).getChildren().add(L);

}


    public void resetMapView() {
        ((AnchorPane) mapView.getParent()).getChildren().clear();
    }
...
//    }
    ...
    private void readCSV() {
        ...
        String coordsFileLine;
        File coordinateFile = new File("src/main/resources/com/example/londonundergroundroutefinder/CSV/station_coords.csv");
        try (BufferedReader br = new BufferedReader(new FileReader(coordinateFile))){
            while ((coordsFileLine = br.readLine()) !=null){
                String[] elements = coordsFileLine.split(",");
                System.out.println(elements[4]);
                Station stationName = findGraphNodeByName( elements[1].trim()).data;
                stationName.setX(Integer.parseInt(elements[3].trim()));
                stationName.setY(Integer.parseInt(elements[4].trim()));
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        removeLinesButton.setOnAction(e -> resetMapView());
